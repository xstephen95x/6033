...
class MapReduce(object):
    ...
    def doMap(self, i):
        ...
        keyvalue = f.readline() ## reads first line
        value = f.read() ## reads rest
        f.close() ## closes file
    ...
    def doReduce(self, i):
        ...
        for m in range(0, self.maptask):
            ...
            for item in itemlist: ## item = (word,count) = ("jesus",1)
                if keys.has_key(item[0]):
                    keys[item[0]].append(item) ## keep all instances in a list
                else:
                    keys[item[0]] = [item] # word = [](word,1)
            f.close()
            os.unlink("#map-%s-%s-%d" % (self.path, m, i))
        for k in sorted(keys.keys()): #iterate words alphabetically
    ...
    # The master.
    def run(self):
        pool = Pool(processes=max(self.maptask, self.reducetask),) # number of processes
    ...
    def Map(self, keyvalue, value):
        ...
        while i < n:
            ...
            while i < n and value[i] in string.ascii_letters:
                i += 1
            w = value[start:i]
            if start < i and w.istitle(): # if this word is capitalized
